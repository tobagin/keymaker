# Subprocess Handling Specification Delta
...
### Timeout Values
The following timeout values are established based on operation characteristics:

| Operation Type | Timeout | File | Justification |
|---------------|---------|------|---------------|
| Connection diagnostics | 10s | ConnectionDiagnostics.vala | Network operations may be slow on poor connections |
| QR code operations | 15s | EmergencyVault.vala | QR processing can be computationally intensive |
| ssh-keygen operations | 5s | Metadata.vala | Local filesystem operations should be fast |

### Error Messages
Error messages from failed subprocess operations SHALL:
- Include the stderr output from the command
- Indicate the type of operation that failed
- Provide actionable guidance where possible (e.g., "Install zbar-tools package")
- Use user-friendly language (not raw technical errors)

### Migration Strategy
When migrating existing code:
1. Identify the subprocess call and its purpose
2. Determine appropriate timeout based on operation type
3. Replace with corresponding Command utility method
4. Update error handling to use KeyMakerError types
5. Verify stdout/stderr parsing still works
6. Test both success and failure paths

### Backward Compatibility
This change maintains backward compatibility at the user level:
- All functionality continues to work identically
- Error messages may improve but remain clear
- Timeout protection prevents UI hangs (improvement, not breaking change)
